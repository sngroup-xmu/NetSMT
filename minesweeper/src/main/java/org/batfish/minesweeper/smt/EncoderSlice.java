...
class EncoderSlice {
  ...
  private void addBoundConstraints() {
    ...
    for (SymbolicRoute e : getAllSymbolicRecords()) {
      ...
      if (e.getLocalPref() != null) {
        ...
        // ADD_BEGIN
        if(_optimizations.ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION){
          int localPrefSize = _optimizations.getLocalPrefAbstraction().get(e.getRouter()).size() +1;
          add(mkLt(e.getLocalPref(), mkInt(localPrefSize+1)));
        }
        else {
  ...
  public BoolExpr equal(
    ...
    // ADD_BEGIN
    Long localPref = (long) defaultLocalPref();
    if(_optimizations.ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION){
      localPref = _optimizations.getAbstractLocalPrefValue(conf.getHostname(), localPref);
    }
    ArithExpr defaultLocal = mkInt(localPref);
    // ADD_END
    ...
    if(vars.getName().contains("_BGP_EXPORT")){
      equalLp = equalHelper(best.getLocalPref(), null, defaultLocal);
    }
    //ADD_END
  ...
  private BoolExpr greaterOrEqual(
    ...
    // fix bug of minesweeper, the local pref can not transite between ebgp link
    if(vars.getName().contains("_BGP_EXPORT")){
      betterLp = geBetterHelper(best.getLocalPref(), null, defaultLocal, false);
      equalLp = equalHelper(best.getLocalPref(), null, defaultLocal);
    }
    //ADD_END
  ...
  //ADD_BEGIN
  HeaderSpace getHeaderSpace(){return _headerSpace;}
  //ADD_END
}
