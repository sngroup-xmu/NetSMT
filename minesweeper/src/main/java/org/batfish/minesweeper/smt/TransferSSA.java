...
class TransferSSA {
  ...
  private BoolExpr matchFilterList(RouteFilterList x, SymbolicRoute other) {
    ...
    for (RouteFilterLine line : lines) {
      ...
      // ADD_BEGIN
      if(_enc.getOptimizations().ENABLE_MATCH_IP_OPTIMIZATION){
        boolean isUsedForDstIp = false;
        for (Prefix dstPrefix: _enc.getOptimizations().getDstPrefixes()){
          if (range.includesPrefixRange(PrefixRange.fromPrefix(dstPrefix))){
            isUsedForDstIp = true;
            break;
          }
        }
          ...
          if (start == end && start == pfx.getPrefixLength()) {
            ...
            if (origin != null && origin.contains(pfx)) {
              ...
              if(_enc.getOptimizations().ENABLE_MATCH_IP_OPTIMIZATION){
                boolean isUsedForDstIp = false;
                for (Prefix dstPrefix: _enc.getOptimizations().getDstPrefixes()){
                  if (r.includesPrefixRange(PrefixRange.fromPrefix(dstPrefix))){
                    isUsedForDstIp = true;
                    break;
                  }
                }
  ...
  private ArithExpr applyLongExprModification(ArithExpr x, LongExpr e) {
    if (e instanceof LiteralLong) {
      ...
      // ADD_BEGIN
      //todo: only implement local pref abstraction of LiteralLong now
      Long localPref = z.getValue();
      if(_enc.getOptimizations().ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION){
        localPref = _enc.getOptimizations().getAbstractLocalPrefValue(_conf.getHostname(), localPref);
      }
      return _enc.mkInt(localPref);
      // ADD_END
    }
  ...
  private BoolExpr relateVariables(
    ...
    // ADD_BEGIN
    Long defaultLocalPref = (long) _enc.defaultLocalPref();
    if(_enc.getOptimizations().ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION){
      defaultLocalPref = _enc.getOptimizations().getAbstractLocalPrefValue(_conf.getHostname(), defaultLocalPref);
    }
    ArithExpr defaultLp = _enc.mkInt(defaultLocalPref);
    // ADD_END
    ...
    // ADD_BEGIN
    // fix bug of minesweeper, the local pref can not transite between ebgp link
    if(_other.getName().contains("_BGP_EXPORT")){
      otherLp = defaultLp;
    }
    // ADD_END
      ...
      if (stmt instanceof StaticStatement) {
        ...
        switch (str) {
          ...
          case "false":
            ...
            // ADD_BEGIN
            // fix bug of minesweeper, do not overwrite before change
            curResult = compute(i.getFalseStatements(), curP.indent(), curResult);
            // ADD_END
        ...
        for (CommunityVar cvar : comms) {
          // ADD_BEGIN
          if(_enc.getOptimizations().ENABLE_MATCH_COMMUNITY_OPTIMIZATION &&
                  !curP.getData().getCommunities().containsKey(cvar)){
            continue;
          }
          // ADD_END
  ...
  private void setDefaultLocalPref(TransferParam<SymbolicRoute> p) {
    ...
    if (p.getData().getLocalPref() == null) {
      ...
      if (Optimizations.ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION){
        Long abstractLP = _enc.getOptimizations().getAbstractLocalPrefValue(_conf.getHostname(), 100L);
        p.getData().setLocalPref(_enc.mkInt(abstractLP));
      }
      else {
